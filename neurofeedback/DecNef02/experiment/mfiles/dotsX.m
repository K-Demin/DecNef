function [frames, rseed, start_time, end_time, response, response_time,choice,outcome] = dotsX(screenInfo, dotInfo, targets,gambleParam,results)
%
%
...
% IF YOU HAVE A MAC (AKA MOUSLIM) YOU NEED THIS TO BE SAVED AS
% dotsXphase2.m FOR THE BUTTON BOX TO WORK CORRECTLY
%
...
coh = gData.dot.coh/1000;	%  % dotInfo.coh is specified on 0... (because
...
if isempty(gData.dot.Ls)
    for df = 1 : gData.dot.numDotField
        % dxdy is an N x 2 matrix that gives jumpsize in units on 0..1
        %    	 deg/sec     * Ap-unit/deg  * sec/jump   =   unit/jump
        dxdy{df} 	= repmat((gData.dot.speed(df)/10) * (10/apD(df)) * (3/gData.dot.monRefresh) ...
            * [cos(pi*gData.dot.dir(df)/180.0) -sin(pi*gData.dot.dir(df)/180.0)], ndots(df),1);
...
%     Screen('FillOval', gData.dot.curWindow, targets.colors(i,:), targets.rects(i,:));
% end
...
%         end
        ...
        if sum(L) > 0
            xdir = sin(pi*gData.dot.dir(df)/180.0);
            ydir = cos(pi*gData.dot.dir(df)/180.0);
