function GenerateConditionSPM(log,task)

%%%%
% This will generate the condition files for SPM
% Which need to contain 
% names
% duration
% onsets
% pmod
...
for i = 1:length(pain)
    ...
    TimePain(i) = str2double(log{pain(i)}(posCol+1:end));
end
onsets{2} = TimePain;
...
for i = 1:length(noPain)
    ...
    TimeNoPain(i) = str2double(log{noPain(i)}(posCol+1:end));
end
onsets{5} = TimeNoPain;
...
for i = 1:length(noPain)
    % Findtherating immediately after the pain rating
    posCol = findstr(log{noPain(i)+2},':');
    posSpace = findstr(log{noPain(i)+2},' ');
